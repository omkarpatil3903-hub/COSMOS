...
import { getStatusBadge, getPriorityBadge } from "../../utils/colorMaps"; // Adjust path if necessary
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(assigneesResolved || task.assignees).map((a, idx) => {
                      ...
                      return (
                        <span
                          key={idx}
                          className="inline-flex items-center gap-1.5 rounded-md bg-gray-200 px-2.5 py-1 text-xs font-semibold text-gray-800"
                          title={titleText}
                        >
                          👤 {a.name || a.id}
                        </span>
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                {task.okrObjective && (
                  <p className="text-sm text-green-900 font-semibold mb-2">
                    🎯 Objective:{" "}
                    <span className="font-medium">{task.okrObjective}</span>
                  </p>
                )}
                {Array.isArray(task.okrKeyResults) &&
                  task.okrKeyResults.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {task.okrKeyResults.map((kr, idx) => (
                        <span
                          key={idx}
                          className="inline-flex items-center gap-1.5 rounded-md bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-800"
                          title={`KR: ${kr}`}
                        >
                          ✅ {kr}
                        </span>
