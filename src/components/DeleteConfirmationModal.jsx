    ...
    <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md mx-4">
      <div className="flex items-start gap-4 mb-4">
        ...
        <div className="flex-1">
          ...
          {itemTitle && (
            <div className="mb-2">
              <p className="text-sm font-semibold text-gray-900">{itemTitle}</p>
              {itemSubtitle && (
                <p className="text-xs text-gray-600 mt-0.5">{itemSubtitle}</p>
              )}
            </div>
          )}
          <p className="text-gray-600 text-sm">{mainDescription}</p>
          <p className="text-gray-500 text-xs mt-2">{secondaryMessage}</p>
        </div>
      </div>
