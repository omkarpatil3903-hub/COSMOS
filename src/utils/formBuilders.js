 ...
 * @returns {object}
 */
export const transformEventFormToDoc = (form, selectedAttendees, clientsById) => {
  const client = clientsById.get(form.clientId);
  const attendeeNames = selectedAttendees
    .map((id) => {
      const attendee = Array.from(clientsById.values()).find((c) => c.id === id);
      return attendee?.clientName || attendee?.companyName || attendee?.name || "";
    })
    .filter(Boolean);
